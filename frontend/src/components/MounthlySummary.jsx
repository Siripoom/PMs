...
import { Card, Table, Tabs, Empty, Spin, Row, Col, Select, Statistic } from 'antd';
import { 
...
const MonthlySummary = ({ supabase }) => {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const yearsList = [...new Set(projects.map(p => moment(p.created_at).year()))];
      setYears(yearsList);
  ...
  const calculateYearlyStats = () => {
    const totalProjects = monthlyData.reduce((sum, month) => sum + month.projectCount, 0);
    const totalBudget = monthlyData.reduce((sum, month) => sum + month.totalBudget, 0);
      ...
      <Card 
        title={
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span><BarChartOutlined /> สรุปรายเดือน</span>
            <Select 
              value={selectedYear} 
              ...
              <div style={{ height: 400 }}>
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Legend />
                    <Bar yAxisId="left" dataKey="projectCount" name="จำนวนโปรเจค" fill="#8884d8" />
                    <Bar yAxisId="right" dataKey="totalBudget" name="งบประมาณ" fill="#82ca9d" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
              ...
              <div style={{ height: 400 }}>
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Legend />
                    <Line yAxisId="left" type="monotone" dataKey="projectCount" name="จำนวนโปรเจค" stroke="#8884d8" activeDot={{ r: 8 }} />
                    <Line yAxisId="right" type="monotone" dataKey="totalBudget" name="งบประมาณ" stroke="#82ca9d" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            ) : (
                    ...
                    {record.projects.length > 0 ? (
                      <ul>
                        {record.projects.map(project => (
                          <li key={project.id}>
                            {project.name} - {project.budget?.toLocaleString('th-TH')} บาท
